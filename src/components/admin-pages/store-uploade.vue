...
const patient = ref({
  src:avatar1,
        ...
        price: '',
        description: '',
        // image: 'https://example.com/iphone-xr.jpg',
        shopLink: ''
});
...
const addPatient = async () => {
  ...
  try {
    ...
    const patientsCollectionRef = collection(db, "store");
  ...
  <VRow v-if="done">
    <VCol cols="25" style="padding:3pc">
     <a href="/AiToolsList" class="link"> <h2>
      ...
      <VCard style="padding:2pc"  append-icon="mdi-instagram" title="Add Product">
  ...
  <v-row>
    ...
    <v-row>
      <v-form ref="productForm" v-model="valid" lazy-validation>
        <v-text-field v-model="patient.name" label="Name" required></v-text-field>
        <v-text-field v-model="patient.price" label="Price" required></v-text-field>

    <v-text-field v-model="patient.shopLink" label="ShopLink" required></v-text-field>
    <!-- <v-file-input v-model="patient.image" label="Product Image" required></v-file-input> -->
    <v-textarea v-model="patient.description" label="Description" required></v-textarea>
    <v-btn color="primary" class="mr-4" @click="uploadImage" :disabled="!valid">Submit</v-btn>
